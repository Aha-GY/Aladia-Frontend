<template>
  <div class="mb-4 rounded bg-white/5 p-4">
    ...
    <el-popover ref="languages" trigger="click" width="356" placement="bottom">
      <template #reference>
        <BaseInput
          v-model="name"
          fixed
          clear
          class="mb-4 text-sm"
          :placeholder="$t('personal.language.select')"
          prefix-icon-class="fa-solid fa-rectangle-history-circle-plus"
          data-cy="course-personal-language"
        />
      </template>
      <div
      ...
      >
        ...
        >
          ...
          <BaseCheckboxStyle
            :state="isChecked(item.id) ? 'correctChecked' : 'unchecked'"
          />
        </div>
      </div>
    </el-popover>
...
export default {
  ...
  computed: {
    languages() {
      const result = [...this.searchStore.coursesLanguages].sort((prev, curr) =>
    ...
    selecteds() {
      return this.searchStore.coursesLanguages.filter((item) =>
        this.languageIds.includes(item.id),
      );
    },
    preview() {
    ...
    languageIds() {
      return this.query.languageIds;
    },
  },
  mounted() {
    this.searchStore.searchCoursesLanguages({
      subCategoryId: this.$route.query.subCategoryId || undefined,
    });
  },
  methods: {
    onLanguageClick(item) {
      if (this.languageIds.includes(item.id)) {
        const index = this.languageIds.indexOf(item.id);
        this.languageIds.splice(index, 1);
      } else {
        this.languageIds.push(item.id);
      }
    },
    ...
    isChecked(id) {
      return this.languageIds.includes(id);
    },
  },
};
</script>
