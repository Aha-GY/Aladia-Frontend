  ...
  <div class="relative flex h-full flex-col">
    ...
    >
      <div v-for="key in Object.keys(groupDataList)" :key="key">
        <div class="px-2 py-2 text-xs">{{ key }}</div>
        <div class="grid grid-cols-5 gap-1 px-2">
          <OrganizationHubChatRightSideMediaItem
      ...
      <div v-show="loading" class="grid grid-cols-5 gap-1 px-2 py-1">
        <BaseSkeleton
          v-for="i in 20"
          :key="i"
          height="5rem"
          class="h-20 w-20 rounded-5"
          :length="1"
        />
      </div>
    </div>
  ...
  methods: {
    async load() {
      ...
      const {
        data,
        cursor: { next },
      } = await this.hubChatStore.searchRoomMedia({
    ...
    async onDelete(deleteForAll) {
      ...
      // const attachments = this.dataList
