<template>
  ...
  >
    ...
    >
      {{
        selecteds.length === 0
          ? $t('category.transaction.selectTopics')
          : $t('category.transaction.selectTopicsCount', {
    ...
    <div v-if="selecteds.length" class="mr-1.5 flex items-center">
      ...
      >
        <BaseCourseThumbnail class="h-[1.75rem]" :url="item.image?.url" />
      </div>
  ...
  >
    ...
    >
      <BaseCategorySelected :selecteds="selecteds" @remove="onRemove" />
      <div
      ...
      >
        ...
        >
          ...
          <div
            ...
            v-html="highlightKeywords(item.name, [search])"
          />
        </div>
...
<script setup>
import { searchCategories } from '~/api/v2/category';
...
const getData = async () => {
  try {
    loading.value = true;
    const { data, cursor } = await searchCategories({
      name: search.value,
      next: next.value,
      limit: 20,
    });
