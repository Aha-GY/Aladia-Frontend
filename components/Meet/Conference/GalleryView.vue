<template>
  ...
  >
    ...
    <div
      ...
      :class="[
        isExpand
          ? `h-full ${allWidth < 960 ? 'w-full overflow-x-auto' : 'hide-scrollbar w-[16.5rem] flex-col overflow-y-auto'} `
    ...
    >
      ...
      <!-- <div
        v-if="!expand && isExpand && morePeers.length"
        class="relative flex shrink-0 cursor-pointer flex-col items-center justify-center overflow-hidden rounded-5 bg-[#202020]"
        :class="allWidth < 960 ? 'gap-1' : 'gap-5'"
        :style="{
          width: `${width}px`,
          height: `${height}px`,
      ...
      >
        <div class="flex items-center justify-center">
          <div
            v-for="peer in morePeers.slice(0, 2)"
            :key="peer.id"
            class="-ml-1 size-10 rounded-full border border-[#202020] first:ml-0"
          >
            <BaseAvatar
        ...
        <div class="text-sm text-[#FAFAFA]">
          {{ morePeers.length }} {{ $t('meet.others') }}
        </div>
      </div> -->
...
export default {
  ...
  setup() {
    ...
    return { hmsStore, allWidth: width };
  },
  data() {
    return {
      pageNumber: 1,
