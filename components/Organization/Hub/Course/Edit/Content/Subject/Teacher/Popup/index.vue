<template>
  <div class="flex h-full w-full flex-col justify-between bg-[#000000] pt-3">
    <div class="px-4">
      <BaseInput
        v-model="username"
        search
    ...
    <div
      v-if="open"
      class="hide-scrollbar h-full overflow-y-auto bg-[#000000] px-1"
    >
      <OrganizationHubCourseEditContentSubjectTeacherPopupItem
        v-for="member in membersSort"
        :key="member.id"
        :member="member"
      />
    </div>
    <div v-else class="hide-scrollbar h-full overflow-y-auto bg-[#000000] px-1">
      <OrganizationHubCourseEditContentSubjectTeacherPopupPreview
        v-for="member in membersSort"
        :key="member.id"
        :member="member"
      />
    ...
    <div
      class="flex justify-between bg-[#0C0C0C] px-1 py-2 text-sm font-normal"
    >
      <button
        class="flex items-center gap-2 rounded-md bg-[#A20F0F]/10 p-2 py-1.5 text-[#D9D9D9] hover:bg-[#A20F0F]/15"
        @click="hubSubject.isTeacherPoup = false"
      >
        <i class="fa-light fa-reply text-sm" />
        <span class="text-sm">cancel</span>
      </button>
      <button
        class="flex items-center gap-2 rounded-md p-2 py-1.5 text-[#707070] hover:bg-white/5"
        @click="onSave"
      >
