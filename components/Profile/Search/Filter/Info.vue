    ...
    <div class="text-sm">
      ...
      >
        <template #reference>
          <BaseInput
            v-model="query.skill"
            :title="$t('personal.skill')"
            @click="skillFocus = true"
            @blur="skillFocus = false"
          />
        </template>
        <div class="flex h-72 flex-col overflow-y-auto p-1">
          <div
            ...
            :key="item.id"
            class="cursor-pointer rounded px-2 py-1 hover:bg-white/10 hover:text-white"
            data-cy="ac81c596f598b960"
            @click="onSkillClick(item)"
          >
            {{ item.name }}
          </div>
        </div>
...
export default {
  ...
  computed: {
    skills() {
      const result = [...this.searchStore.skills].sort((prev, curr) =>
        prev.name.localeCompare(curr.name),
      );

      if (
        this.query.skill &&
        result.every((item) => item.name !== this.query.skill)
      ) {
        return result.filter((item) =>
          item.name.toLowerCase().includes(this.query.skill.toLowerCase()),
        );
      }
