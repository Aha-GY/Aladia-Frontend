<template>
  <div class="bg-black p-8">
    <div class="mb-4 flex items-center gap-4">
      <BaseButton data-cy="eba7b0412b9f0fd5" @click="categoryPopup = true">
        <i class="fa-solid fa-plus" />
        {{ $t('category.add.sub') }}
      </BaseButton>
      <div v-if="categories.cursor" class="text-xs text-white/50">
        {{ categories.cursor?.totalItems }} {{ $t('category.subName.lower') }}
      </div>
    </div>
    <el-table
      v-loading="loading"
    ...
    >
      <el-table-column
        prop="name"
        :label="$t('category.subName.upper')"
        width="200"
      />
      ...
      <div class="flex flex-col">
        ...
        >
          <div class="flex items-center gap-2 text-lg">
            {{ $t('category.subName.upper') }}
          </div>
          <BaseIconClose
            data-cy="050e5ecf6603c3ee"
        ...
        <div class="flex flex-col gap-5 p-3">
          <div>
            ...
            <el-input
              ...
              :placeholder="$t('category.input.hint.sub')"
            />
          </div>
          <div>
            <div class="mb-2 text-sm text-white">
              {{ $t('category.form.icon.label') }}:
            </div>
            <div class="mb-2 flex items-center gap-2">
              <BaseButton
                class="inline-flex"
                data-cy="b7f9b4fffd72d6ec"
                @click="open"
              >
        ...
        >
          <BaseButton
            type="tertiary"
            data-cy="20d38edf3701b72d"
            @click="categoryPopup = false"
          >
            {{ $t('base.upload.cancel') }}
          </BaseButton>
          <BaseButton data-cy="6e55da3b2398ec0c" @click="onCategoryConfirm">
  ...
  methods: {
    ...
    async handleCategoryDelete(category) {
      await ElMessageBox.confirm(
        `"${$t('category.dialog.sub')}" "${category.name}"?`,
        `${$t('base.editor.delete')}"`,
        {
