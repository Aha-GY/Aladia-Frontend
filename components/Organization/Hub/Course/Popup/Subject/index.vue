<template>
  <div
    v-if="hubSubject.subjectPopup"
    ref="popupRef"
    class="subjectpopup absolute left-0 right-0 !z-50 mx-auto flex h-full w-full flex-col justify-between rounded-lg border border-white/20 bg-[#0c0c0c] py-1"
  >
    ...
    >
      <div
        ...
        @click="
          ...
          if (!hubCourse.lecturePopup) hubCourse.popupOpen = false;
...
export default {
  setup() {
    ...
    onClickOutside(
      ...
      () => {
        ...
        hubCourse.subjectOpen = false;
        if (!hubCourse.lecturePopup) {
          hubCourse.popupOpen = false;
        }
      },
      {
        ignore: ['.el-popper', '.pcr-app'],
      },
    );
    return {
      hubSubject,
  ...
  watch: {
    'hubSubject.addNewSubject'(val) {
      if (val) {
        this.hubSubject.addNewSubject = false;
        this.onAdd();
      }
    },
  },
