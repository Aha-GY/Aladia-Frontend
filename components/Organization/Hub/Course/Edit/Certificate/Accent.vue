  ...
  <div class="mb-4 flex w-full cursor-pointer items-center gap-2 text-sm">
    <div class="flex size-5 items-center justify-center">
      <div
        ...
        :style="{
          backgroundColor: placeholder.accentColor,
        }"
      />
    </div>
    <el-popover
    ...
    >
      <template #reference>
        ...
        >
          <BaseInput
            ref="color"
            v-model="placeholder.accentColor"
            readonly
            title="Accent a color"
          />
          <img src="~/assets/icons/color-pick.svg" class="h-6" />
        </div>
      </template>
      <div>
        <LazyBaseColor :color="placeholder.accentColor" @change="onChange" />
        <div class="px-2 pb-2">
...
export default {
  ...
  methods: {
    ...
    onChange(color) {
      this.placeholder.accentColor = color;
    },
  },
};
</script>
