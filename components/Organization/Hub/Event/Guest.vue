<template>
  ...
  >
    <div class="flex w-5 justify-center">
      <i class="fa-light fa-user text-base text-white/20" />
    </div>
    <div
    ...
    >
      <div
        ...
        data-cy="ce7250213f7c1fb4"
        @click="total && (open = !open)"
      >
        {{ $t('meet.roles.guest', { total: total }) }}
        <i
          v-if="total"
...
export default {
  ...
  computed: {
    ...
    exclude() {
      ...
      const coHostIds = this.event.addedMembers
        .filter((r) => r.role === 'co-host')
        .map((r) => r.profileId);
      return [...hostIds, ...coHostIds, this.authStore.user.id];
    },
    total() {
      const guestNumber = this.event.guestNumber || 0;
      const addLength = this.addedMembers.length;
      const removeLength = this.removedIds.length;
      return Math.max(0, guestNumber + addLength - removeLength);
    },
    removedIds() {
    ...
    addedMembers() {
      return this.event.addedMembers.filter((r) => r.role === 'guest');
    },
    guestMembers() {
      return this.event.guestMembers || [];
    },
    allMembers() {
      const guestMembers = this.guestMembers.filter(
        (r) => !this.removedIds.includes(r.id),
      );
      return [...guestMembers, ...this.addedMembers].slice(0, 4);
    },
    inviteParams() {
  ...
  methods: {
    onRemoveClick(member) {
      ...
      // this.event.guests = this.event.guests.filter(
      //   (item) => item.id !== member.id,
      // );
    },
    onSave(selecteds) {
      // this.event.guests = [...selecteds];
