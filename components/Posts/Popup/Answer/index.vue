  ...
  <div class="mb-3 last:mb-0">
    <div ref="main" class="main-box">
      <div v-if="item.skeleton" class="relative">
        ...
        <PostsPopupSkeleton :avatar-size="level < 2 ? 'md' : 'sm'" />
      </div>
      <div v-else class="relative flex items-start gap-2">
        ...
        <div class="w-full flex-1">
          <div
            class="--mention-link-enabled group relative inline-flex min-h-[3rem] max-w-full flex-col rounded-[0.3125rem] border bg-[#242526] px-3 py-[0.625rem] text-13 transition-all duration-300"
            :class="{
              'border-white/50 bg-[#313131]': isReplying,
              'border-transparent bg-[#101010]': !isReplying,
            }"
          >
            <PostsPopupAnswerHeader
              :item="item"
              :level="level"
              data-cy="post-popup-comment"
              ...
              @follow="handleFollow"
            />
            <PostsItemWidgetsCommentCollabsableBubble
              v-if="item.content"
              :item="item"
              class="text-[#B0B0B0]"
            ...
            <SocialCommentsAnswerGif :item="item" />
          </div>
          <PostsPopupAnswerAttachments :item="item" />
          <PostsItemWidgetsLooms
          ...
          <div class="flex items-center justify-between px-2">
            <div class="flex items-center gap-1 text-xs font-medium">
              <div
                ...
                class="group relative flex h-7 w-10 cursor-pointer items-center justify-center gap-2 rounded leading-none text-[#FAFAFA] transition-all active:scale-95"
                :class="
                  repliesOpen
                    ? 'font-semibold'
                    : 'font-normal hover:font-semibold'
                "
                data-cy="a630f301546a1e4d"
              ...
              <div
                class="relative flex h-7 w-10 cursor-pointer items-center justify-center gap-2 rounded leading-none text-[#FAFAFA] transition-all duration-300"
    ...
    <div
      ...
      :class="[
        ...
        level > 2 ? 'left-4 top-[2rem] w-[1.9rem]' : 'left-8 top-[3.5rem] w-10',
      ]"
    />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    isCommentAllowed() {
      return !!this.question.isCommentAllowed;
    },
  },
  watch: {
    'socialStore.answerToId'(id) {
      ...
      } else if (this.item.replies?.some((r) => r.id === id)) {
        this.item.numberOf.replies += 1;
      }
    },
  ...
  methods: {
    ...
    handleUserClick() {
      ...
      const profileInfo = {
        ...this.item.owner,
        entityType:
          this.item.owner?.type === 'organization' ? 'organization' : 'user',
      };
      this.profileStore.gotoProfilePage(profileInfo);
    },
