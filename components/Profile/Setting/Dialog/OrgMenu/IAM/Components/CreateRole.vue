    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      <div class="flex items-end justify-end py-1">
        <BaseButton
          ...
          @click="handlerSend"
        >
          <span>{{ $t('org.group.invite.label') }}</span>
        </BaseButton>
      </div>
    </div>
    <div
...
const inviteBtnDisabled = computed(() => {
  ...
  return props.type === 'group'
    ? inviteBtnDisabled || !props.groupDetail?.id
    : inviteBtnDisabled;
});
...
const commonParams = computed(() => ({
  ...
  spaces:
    selectSpaceCoursesValue.value.spaces
  ...
  courses:
    selectSpaceCoursesValue.value.courses
      ?.filter((item) => item.role)
      .map((item) => ({
        id: item.id,
...
watch(
  () => [activeRole.value],
  () => {
    if (activeRole.value === 'organization-admin') {
      billingAndDeveloper.value.forEach((item) => {
        item.checked = true;
      });
    }
